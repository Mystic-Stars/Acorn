...
`comments.ts` 中的 `repoId` 和 `categoryId` 是公开标识符，不是密钥。`giscus.json` 将生产域名和本地开发地址加入允许来源；新增正式域名时也必须同步更新该文件。评论主题位于 `public/styles/giscus-island.css`，由 giscus iframe 载入；`edgeone.json` 已为该文件配置跨域响应头及 `no-cache`，以确保主题更新时会重新验证。
