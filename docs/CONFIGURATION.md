...
文章页评论由 [giscus](https://giscus.app/zh-CN) 提供。评论、回复和表情反应保存到 GitHub Discussions，因此访客需要使用 GitHub 登录；审核、删除、置顶和封禁在仓库的 **Discussions** 页面完成。

当前配置使用公开仓库 `Mystic-Stars/Acorn` 的 `General` 分类。启用前，请在 GitHub 完成以下操作：

1. 在仓库设置中启用 **Discussions**。
2. 安装 [giscus GitHub App](https://github.com/apps/giscus)，并仅授权此仓库。
3. 如需使用专用分类，在 Discussions 中创建“博客评论”分类，再通过 [giscus 配置器](https://giscus.app/zh-CN) 获取新的 `category` 与 `categoryId`，更新 `src/config/comments.ts`。

`comments.ts` 中的 `repoId` 和 `categoryId` 是公开标识符，不是密钥。`giscus.json` 将生产域名和本地开发地址加入允许来源；新增正式域名时也必须同步更新该文件。评论主题位于 `public/styles/giscus-island.css`，由 giscus iframe 载入；`edgeone.json` 已为该文件配置跨域响应头。修改该样式文件时同步更新 `commentsConfig.themeVersion`，以强制刷新 giscus 与 EdgeOne 的样式缓存。

本地 `http://localhost:4321` 开发时，giscus 的 HTTPS iframe 不能直接读取 HTTP 本地主题文件。评论组件会自动读取本地 CSS，并通过 giscus 的配置消息以 data URL 注入 iframe；无需关闭浏览器安全策略或改用第三方代理。

评论按文章路径关联，例如 `/archives/hello-world/`。发布后的文章文件名即永久链接的一部分，**不要通过 Pages CMS 重命名已发布文章**；否则新路径会建立一条新的讨论。需要迁移链接时，先在 GitHub Discussions 保留或迁移原讨论，再发布新路径。
